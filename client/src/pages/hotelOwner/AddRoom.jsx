...
const AddRoom = () => {
    ...
    return (
    <form>
        <Title title={'Add Room'} subTitle={'Fill in the details carefully and accurate room details,pricing and aminities to enhance the user booking experience '} 
        ...
        <div className="flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm ">
            {Object.keys(inputs.amenities).map((aminity,index)=>(
                <div key={index}>
                    <input type="checkbox" id={`aminities${index+1}`} checked={inputs.amenities[aminity]} onChange={()=>setInputs({...inputs,amenities:{...inputs.amenities,[aminity]:!inputs.amenities[aminity]}})}/>
                    <label htmlFor={`aminities${index+1}`}>{aminity}</label>
                </div>
            ))}
